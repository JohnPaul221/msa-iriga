...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/missiriga`.
...
5. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `missiriga` and import [missiriga.sql](missiriga.sql) into it.

6. Open the terminal and navigate to the project directory **missiriga**.
...
9. Open your web browser and access <http://localhost:5199/missiriga/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-missiriga` and import [missiriga.sql](missiriga.sql) into it.
...
2. Access the application by visiting `http://[host_name]/missiriga`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/missiriga>
     - <http://192.168.1.99/missiriga>


#### Admin Dashboard
Access the **admin dashboard** at <http://localhost/missiriga/app>.
